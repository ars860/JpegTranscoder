...
huffman_tree_node *build_tree(const std::map<ngram, int> &dict) {
    std::multiset<huffman_build_node, decltype([](huffman_build_node const &a, huffman_build_node const &b) {
        return a.cnt < b.cnt;
    })> build_que;
...
std::pair<bit_vector, std::vector<ngram>> huffman_tree_node::encode_tree() {
    std::queue<huffman_tree_node *> que;
    que.push(this);

    bit_vector res;
    std::vector<ngram> letters;
    while (!que.empty()) {
...
std::map<ngram, std::string> huffman_tree_node::get_codes() {
    std::map<ngram, std::string> res;
...
bit_vector huffman_tree_node::encode_tree_with_letters() {
    ...
    for (auto &letter: letters) {
        for (unsigned char ch: letter.data) {
            for (int i = 0; i < 8; i++) {
...
huffman_tree_node::decode_tree(std::string &encoded, int start_pos, bool print_bits) {
    ...
    for (auto current_terminal: terminals_to_init) {
        ...
        for (int j = 0; j < NGRAM_SIZE; j++) {
            ...
            letter.data[j] = letter_ch;
        }

        if (print_bits) {
            std::cout << " : " << letter.show() << '\n';
        }
