    ...
    Decoder::ResultCode Decoder::parseSegmentInfo(const UInt8 byte)
    {
        ...
        switch(byte)
        {
            ...
            case JFIF_SOS  :
                ...
                parseSOSSegment();
                if (only_header) {
                    logFile << "Decoder is in only_header mode, finishing parsing" << std::endl;
    ...
    Decoder::ResultCode Decoder::decodeImageFile()
    {
        ...
        while (m_imageFile >> std::noskipws >> byte)
        {
            ...
            else
            {
                ...
                status = ResultCode::ERRORR;
                break;
            }
        }

        if (status == ResultCode::DECODE_DONE && !only_header)
        {
            decodeScanData();
            // TODO(or not): subsampling shit
//            m_image.createImageFromMCUs(m_MCU);
    ...
    Decoder::ResultCode Decoder::parseSOF0Segment()
    {
        ...
        for (auto i = 0; i < 3; ++i)
        {
            ...
            if (i == 0) {
                subsampling = {int(sampFactor >> 4), int(sampFactor & 0x0F)};
                mcu_comp_cnt = subsampling.first + subsampling.second + 2;
                        // subsampling.first == 2 && subsampling.second == 2 ? 6 : 3;
            }
...
//                isNonSampled = false;
    ...
    void Decoder::parseSOSSegment()
    {
        ...
        headers_length = (size_t) m_imageFile.tellg();
        headers = std::string(headers_length, '0');
        m_imageFile.seekg(start);
        m_imageFile.read(&headers[0], headers_length);
        m_imageFile.seekg(headers_length);
    ...
    void Decoder::decodeScanData()
    {
        ...
        std::map<std::pair<uint8_t, uint8_t>, std::vector<std::string>> subsampling2component = {
                {{1, 1}, {"Y (Luminance)", "Cb (Chrominance)", "Cr (Chrominance)"}},
//                {{2, 1}, {"Y0", "Y1", "Cb", "Cr"}},
                {{2, 2}, {"Y00", "Y01", "Y10", "Y11", "Cb", "Cr"}}
        };
//        const char* component[] = { "Y (Luminance)", "Cb (Chrominance)", "Cr (Chrominance)" };

        const char* type[] = { "DC", "AC" };

        // may be need to do a little switcharoo
        int mcu_width = 8 * subsampling.first;
        int mcu_height = 8 * subsampling.second;
        int padded_image_width = m_image.width + (m_image.width % mcu_width == 0 ? 0 : mcu_width - (m_image.width % mcu_width));
        int padded_image_height = m_image.height + (m_image.height % mcu_height == 0 ? 0 : mcu_height - (m_image.height % mcu_height));
        int MCUCount = (padded_image_width * padded_image_height) / (mcu_width * mcu_height);
//        MCUCount /= subsampling.first;
//        MCUCount /= subsampling.second;
...
//            uint8_t comp_cnt = mcu_comp_cnt[]
            // The run-length coding after decoding the Huffman data
            std::vector<std::vector<int>> RLE(mcu_comp_cnt);
            ...
            for (auto compID = 0; compID < mcu_comp_cnt; ++compID)
            {
                std::string bitsScanned = ""; // Initially no bits are scanned

                // Firstly, decode the DC coefficient
                logFile << "Decoding MCU-" << i + 1 << ": " << subsampling2component[subsampling][compID] << "/" << type[HT_DC] << std::endl;

                int HuffTableID = compID >= mcu_comp_cnt - 2 ? 1 : 0; // 0 table for Y, 1 for Cb and Cr
//                logFile << "With HTC: " << HuffTableID << std::endl;
                ...
                while (1)
                {
                    ...
                    if (!utils::isStringWhiteSpace(value))
                    {
                        if (value != "EOB")
                        {
                            ...
                            // logFile << std::dec << "code: " << bitsScanned << " value: " << value << " zero count: " << zeroCount << " category: " << category << " DC: " << DCCoeff << std::endl;
                ...
                logFile << "Decoding MCU-" << i + 1 << ": " << subsampling2component[subsampling][compID] << "/" << type[HT_AC] << std::endl;
                bitsScanned = "";
                int ACCodesCount = 0;
    ...
    std::vector<std::array<int, 64>> Decoder::get_raw_zigzags() {
        std::vector<std::array<int, 64>> res;
        for (auto &mcu : m_MCU) {
            auto zigzags = mcu.zigzagify();
            res.insert(res.end(), zigzags.begin(), zigzags.end());
        }
        return res;
    }

    void Decoder::decode_only_header() {
        only_header = true;
    }

    void Decoder::start_from(int bytes) {
        if (!m_imageFile.is_open() || !m_imageFile.good() || m_imageFile.tellg() != 0) {
            return;
        }
