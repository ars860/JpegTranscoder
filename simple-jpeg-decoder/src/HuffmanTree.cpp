...
namespace kpeg
{
    ...
    std::map<char, std::string> rle2code_rec(Node* node) {
        if (node == nullptr) {
            return {};
        }

        if (node->leaf) {
//            if (node->value == 0x0000) {
//                return
//            }
            return {{(char) (uint8_t) (node->value), node->code}};
        }

        auto left = rle2code_rec(node->lChild.get());
        auto right = rle2code_rec(node->rChild.get());
    ...
    std::map<char, std::string> HuffmanTree::rle2code() {
        return rle2code_rec(m_root.get());
    }
}
