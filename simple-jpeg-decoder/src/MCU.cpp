...
namespace kpeg {
    ...
    MCU::MCU(const std::vector<std::vector<int>> &compRLE, const std::vector<std::vector<UInt16>> &QTables) {
        m_block.resize(compRLE.size());
        constructMCU(compRLE, QTables);
    }

    void
    MCU::constructMCU(const std::vector<std::vector<int>> &compRLE, const std::vector<std::vector<UInt16>> &QTables) {
        m_QTables = QTables;
        ...
        for (int compID = 0; compID < compRLE.size(); compID++) {
            ...
            for (auto i = 0; i <= compRLE[compID].size() - 2; i += 2) {
                ...
                // WTF dude this shit is not correct
...
//        convertYCbCrToRGB();
    ...
    std::vector<std::array<int, 64>> MCU::zigzagify() {
        std::vector<std::array<int, 64>> result;
        for (int compID = 0; compID < m_block.size(); compID++) {
            std::array<int, 64> zzOrder{};
            // Zig-zag order to 2D matrix order
            for (int i = 0; i < 8; ++i) {
                for (int j = 0; j < 8; j++) {
                    int coord = matIndicesToZZOrder(i, j);

                    zzOrder[coord] = m_block[compID][i][j];
                }
            }
            result.push_back(zzOrder);
        }
